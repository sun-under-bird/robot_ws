...
// 返回统一输出目录；可通过 ROBOT_OUTPUT_DIR 覆盖默认位置。
static std::string GetDefaultOutputDirectory()
{
    const char *output_dir = std::getenv("ROBOT_OUTPUT_DIR");
    if (output_dir != nullptr)
        return output_dir;

    const char *home = std::getenv("HOME");
    return home != nullptr ? std::string(home) + "/robot_ws/output" : "./output";
}
...
int main(int argc, char **argv)
{
    ...
    n->declare_parameter<std::string>("output_path", GetDefaultOutputDirectory());
    global_result_path = n->get_parameter("output_path").as_string() + "/vio_global.csv";
