...
void pubTF(const Estimator &estimator, const std_msgs::msg::Header &header)
{
    ...
    transform.transform.translation.x = correct_t.x();
    transform.transform.translation.y = correct_t.y();
    transform.transform.translation.z = correct_t.z();
    ...
    // 发布相机相对机体系的外参，保持原始 body -> camera 语义。
    ...
    transform_cam.transform.rotation.x = camera_q.x();
    transform_cam.transform.rotation.y = camera_q.y();
    transform_cam.transform.rotation.z = camera_q.z();
    transform_cam.transform.rotation.w = camera_q.w();
