...
// 展开配置路径中的 ${VAR} 和 ~/，避免绑定到开发机用户名。
static std::string ExpandPathVariables(const std::string &path)
{
    std::string result = path;
    std::size_t begin = result.find("${");
    while (begin != std::string::npos)
    {
        const std::size_t end = result.find('}', begin + 2);
        if (end == std::string::npos)
            throw std::runtime_error("Invalid environment variable in path: " + path);

        const std::string name = result.substr(begin + 2, end - begin - 2);
        const char *value = std::getenv(name.c_str());
        std::string replacement;
        if (value != nullptr)
            replacement = value;
        else if (name == "ROBOT_OUTPUT_DIR" && std::getenv("HOME") != nullptr)
            replacement = std::string(std::getenv("HOME")) + "/robot_ws/output";
        else
            throw std::runtime_error("Environment variable is not set: " + name);

        result.replace(begin, end - begin + 1, replacement);
        begin = result.find("${", begin + replacement.size());
    }
...
int main(int argc, char **argv)
{
    ...
    if(argc != 2)
    {
        printf("usage: ros2 run loop_fusion loop_fusion_node <config-file>\n");
        return 0;
    }
    ...
    POSE_GRAPH_SAVE_PATH = ExpandPathVariables(POSE_GRAPH_SAVE_PATH);
    VINS_RESULT_PATH = ExpandPathVariables(VINS_RESULT_PATH);
