...
class StereoSplitNodeRgb : public rclcpp::Node
{
public:
    StereoSplitNodeRgb() : Node("stereo_split_node_rgb")
    {
        ...
        std::string pkg_share =
            ament_index_cpp::get_package_share_directory("stereo_camera_pkg");
        ...
        sub_ = create_subscription<sensor_msgs::msg::Image>(
            "/image_raw",
            qos,
            std::bind(&StereoSplitNodeRgb::imageCallback, this, std::placeholders::_1));

        RCLCPP_INFO(this->get_logger(), "Stereo split node (RGB8) started");
    }
...
private:
    ...
    void imageCallback(const sensor_msgs::msg::Image::SharedPtr msg)
    {
        ...
        if (msg->encoding != sensor_msgs::image_encodings::RGB8)
        {
            RCLCPP_ERROR(
                this->get_logger(),
                "Unsupported encoding: %s, expected rgb8",
                msg->encoding.c_str());
            return;
        }
        ...
        if (half_width <= 0)
        {
            RCLCPP_ERROR(this->get_logger(), "Invalid frame width: %d", frame.cols);
            return;
        }
...
int main(int argc, char** argv)
{
    ...
    rclcpp::spin(std::make_shared<StereoSplitNodeRgb>());
    rclcpp::shutdown();
    return 0;
}